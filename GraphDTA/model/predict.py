...
from GraphDTA.model.utils import URVGraphDataset, escala_global, load_split_txt, initialize_model
...
def plot_split_scatter(
        labels, preds, split_name, save_dir, rmse, pearson, global_min, global_max, model_name
):
    ...
    plt.title(
        f"GraphDTA {model_name} – {split_name}\nRMSE = {rmse:.3f} | Pearson = {pearson:.3f}"
    )
...
def predict(
    ...
    results_df.to_csv(
        os.path.join(output_dir, f"metrics_per_split_{model_name}.csv"), index=False
    )
    ...
    summary_df.to_csv(
        os.path.join(output_dir, f"metrics_summary_{model_name}.csv"),
        index=False
    )
    ...
    np.save(os.path.join(output_dir, f"dta_{model_name}_labels.npy"), all_labels_global)
    np.save(os.path.join(output_dir, f"dta_{model_name}_preds.npy"), all_preds_global)
