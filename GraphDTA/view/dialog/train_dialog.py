...
class TrainGraphDTADialog(QDialog):
    def __init__(self, parent=None):
        ...
        last_model_name = self.settings.value(
            "train_graphdta/model_name", "GINConvNet"
        )
        ...
        self.train_file_input.setText(
            self.settings.value("train_graphdta/last_train_file", "")
        )
        ...
        self.test_file_input.setText(
            self.settings.value("train_graphdta/last_test_file", "")
        )
        ...
        self.val_file_input.setText(
            self.settings.value("train_graphdta/last_val_file", "")
        )
        ...
        self.graph_dir_input.setPlaceholderText(
            "Directory containing GraphDTA .pt files"
        )
        self.graph_dir_input.setText(
            self.settings.value("train_graphdta/last_graph_dir", "")
        )
        ...
        self.output_dir_input.setPlaceholderText(
            "Directory to save trained GraphDTA models"
        )
        self.output_dir_input.setText(
            self.settings.value("train_graphdta/last_output_dir", "")
        )
        ...
        self.batch_size_input.setValue(
            int(self.settings.value("train_graphdta/batch_size", 32))
        )
        self.epochs_input = QSpinBox()
        self.epochs_input.setRange(1, 1000)
        self.epochs_input.setValue(
            int(self.settings.value("train_graphdta/epochs", 50))
        )

        self.n_filters_input = QSpinBox()
        self.n_filters_input.setRange(1, 2048)
        self.n_filters_input.setValue(
            int(self.settings.value("train_graphdta/n_filters", 32))
        )

        self.lr_input = QLineEdit()
        self.lr_input.setPlaceholderText("Learning rate, for example 0.001")
        self.lr_input.setText(
            str(self.settings.value("train_graphdta/lr", "0.001"))
        )

        self.weight_decay_input = QLineEdit()
        self.weight_decay_input.setPlaceholderText(
            "Weight decay, for example 0.00001"
        )
        ...
        self.dropout_input.setValue(
            float(self.settings.value("train_graphdta/dropout", 0.2))
        )
    ...
    def get_inputs(self):
        return {
            ...
            "train_split_file": self.train_file_input.text().strip(),
            "test_split_file": self.test_file_input.text().strip(),
            "val_split_file": self.val_file_input.text().strip(),
            "graph_dir": self.graph_dir_input.text().strip(),
            "output_dir": self.output_dir_input.text().strip(),
            "batch_size": self.batch_size_input.value(),
            "lr": float(self.lr_input.text().strip()),
            "n_filters": self.n_filters_input.value(),
