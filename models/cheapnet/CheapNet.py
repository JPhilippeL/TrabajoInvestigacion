...
class MLP(nn.Module):
    def __init__(self, input_dim, output_dim, drop_rate=0.0):
...
class GIGNBlock(nn.Module):
    def __init__(self, input_dim, output_dim, drop_rate):
        ...
        # Usamos los mismos bloques HIL, pero solo con data.edge_index
        self.gconv1 = HIL(input_dim, output_dim, drop_rate)
        self.gconv2 = HIL(input_dim, output_dim, drop_rate)

    def forward(self, x, data):
        # Ambos usan el mismo edge_index de los grafos URV
        x1 = self.gconv1(x, data, data.edge_index)
        x2 = self.gconv2(x, data, data.edge_index)
        x = (x1 + x2) / 2
        return x
...
class DiffPool(nn.Module):
    def __init__(self, input_dim, output_dim, max_num, red_node, drop_rate):
        ...
        self.gnn_e = DenseGCNConv(
            input_dim, output_dim, improved=True, bias=True
        )
    ...
    def forward(self, x, data):
        # siempre usamos data.edge_index
        adj = to_dense_adj(
            data.edge_index, data.batch, max_num_nodes=self.max_num
        )
        x, mask = to_dense_batch(
            x, data.batch, fill_value=0, max_num_nodes=self.max_num
        )

        s = F.softmax(self.gnn_p(x, adj, mask), dim=-1)
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        Q = Q.view(batch_size, seqlen_q, self.heads, self.head_dim).transpose(
            1, 2
        )  # [batch_size, heads, seqlen_q, head_dim]
        K = K.view(batch_size, seqlen_k, self.heads, self.head_dim).transpose(
            1, 2
        )
        V = V.view(batch_size, seqlen_k, self.heads, self.head_dim).transpose(
            1, 2
        )
        ...
        x = (
            x.transpose(1, 2)
            .contiguous()
            .view(batch_size, seqlen_q, self.hidden_dim)
        )  # [batch_size, seqlen_q, hidden_dim]
        x = x.sum(dim=1)
...
class CheapNet(nn.Module):
    def __init__(
        ...
        self.diffpool1 = DiffPool(
            hidden_dim, hidden_dim, 600, num_clusters[0], drop_rate
        )
        self.diffpool2 = DiffPool(
            hidden_dim, hidden_dim, 600, num_clusters[1], drop_rate
        )
