...
class ProteinGAT(nn.Module):
    def __init__(self, feature_dims, nheads, key_dims, value_dims, update_iters, device):
        ...
        self.res_embedding = nn.Sequential(
            nn.Linear(20, feature_dims, bias=False), nn.LeakyReLU(0.1)
        )
        ...
        self.linear_last = nn.Sequential(
            nn.Linear(2 * feature_dims, feature_dims), nn.LeakyReLU(0.1)
        )
        self.device = device
    ...
    def split(self, q, k, v):
        q = torch.reshape(q, shape=[self.node_size, self.nheads, self.key_dims]).permute(
            [1, 0, 2]
        )  # [h,n,k]
        k = torch.reshape(k, shape=[self.node_size, self.nheads, self.key_dims]).permute(
            [1, 2, 0]
        )  # [h,k,n]
        v = torch.reshape(v, shape=[self.node_size, self.nheads, self.value_dims]).permute(
            [1, 0, 2]
        )  # [h,n,v]
        return q, k, v
...
class LigandGAT(nn.Module):
    def __init__(self, feature_dims, nheads, key_dims, value_dims, update_iters, device):
        ...
        self.linear_last = nn.Sequential(
            nn.Linear(31 + feature_dims, feature_dims), nn.LeakyReLU(0.1)
        )
        # para group
    ...
    def split(self, q, k, v):
        q = torch.reshape(q, shape=[self.bonds_size, 1, self.nheads, self.key_dims]).permute(
            [0, 2, 1, 3]
        )  # [b,1,h,k] -- #[b,h,1,k]
        k = torch.reshape(
            k, shape=[self.bonds_size, self.nei_size, self.nheads, self.key_dims]
        ).permute([0, 2, 3, 1])  # [b,h,k,nei]
        v = torch.reshape(
            v, shape=[self.bonds_size, self.nei_size, self.nheads, self.value_dims]
        ).permute([0, 2, 1, 3])  # [b,h,nei,v]
        return q, k, v
    ...
    def forward(self, fatoms, fbonds, agraph, bgraph, lig_scope):
        ...
        nei_mask = torch.where(bgraph != 0.0, 0.0, -1e7).reshape(
            self.bonds_size, 1, 1, self.nei_size
        )  # [fbonds,1,1,nei]
        for _ in range(self.update_iters - 1):
            ...
            q, k, v = self.split(q, k, v)
            attention_matrix = (
                torch.matmul(q, k) / math.sqrt(self.key_dims) + nei_mask
            )  # [b,h,1,nei]
            attention_matrix = torch.softmax(attention_matrix, dim=-1)
            # print(attention_matrix)
            nei_message = torch.matmul(attention_matrix, v).permute(
                [0, 2, 1, 3]
            )  # [b,h,1,v] -- [b,1,h,v]
            nei_message = self.fbonds_out(torch.mean(nei_message, dim=2)).reshape(
                [self.bonds_size, -1]
            )
...
class ProLig(nn.Module):
    ...
    def forward(self, fresidues, fatoms, res_scope, lig_scope):
        ...
        for (start_res, res_count), (start_atom, atom_count) in zip(res_scope, lig_scope):
            ...
            pocket_features = torch.unsqueeze(
                fresidues[start_res : start_res + res_count], dim=0
            )  # [1,res,features]
            ligand_features = torch.unsqueeze(
                fatoms[start_atom : start_atom + atom_count], dim=1
            )  # [atoms,1,features]
            ...
            predicted_lig_interaction = self.linear_lig_interaction_last(
                torch.multiply(
            ...
            predicted_interaction = self.pro_lig_interaction(
                torch.multiply(
            ...
            predict_affinity = torch.sum(
                self.linear_affinity(complex_features) * predicted_interaction
            ).reshape([1])
...
class ProteinEGNN(nn.Module):
    def __init__(self, feature_dims, update_iters, device):
        ...
        self.linear_inf = nn.Sequential(nn.Linear(feature_dims, 1), nn.Sigmoid())
    ...
    def forward(self, fresidues, coordinates, res_scope):
        ...
        for _ in range(self.update_iters):
            ...
            for start_res, res_count in res_scope:
                ...
                fresidues_batch = (
                    self.linear_h(torch.cat([fresidues_batch, m_pre], dim=-1)) + fresidues_batch
                )
    ...
    def pre_cal_res_features(self, fresidues, coordinates):
        ...
        for _ in range(self.update_iters):
            ...
            m = self.linear_e(m_input)
            e = self.linear_inf(m)
            mask = torch.ones([fresidues.shape[0], fresidues.shape[0]]) - torch.eye(
                fresidues.shape[0]
            )
            mask = mask.unsqueeze(-1).to(self.device)
            e = e * mask
...
class ProteinLigandAttention(nn.Module):
    def __init__(self, feature_dims, nheads, update_iters, device):
        ...
        self.update_iters = update_iters
        self.device = device
        self.lig_transform = nn.Sequential(
            nn.Linear(feature_dims, feature_dims, bias=False), nn.LeakyReLU(0.1)
        )
    ...
    def forward(self, fatoms, fresidues, lig_scope, res_scope):
        ...
        for _ in range(self.update_iters):
            ...
            for (start_res, res_count), (start_atom, atom_count) in zip(res_scope, lig_scope):
                ...
                fatoms_batch = (
                    self.lig_out(self.split_update(atoms_q, res_k, res_v)) / res_count
                    + fatoms_batch
                )
                fresidues_batch = (
                    self.res_out(self.split_update(res_q, atom_k, atom_v)) / atom_count
                    + fresidues_batch
                )
