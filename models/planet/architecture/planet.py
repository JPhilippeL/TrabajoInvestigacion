...
from models.planet.architecture.layers import LigandGAT, ProLig, ProteinEGNN
from models.planet.architecture.nnutils import create_var, create_var_gpu
...
class PLANET(nn.Module):
    def __init__(
        ...
        self.ligandgat = LigandGAT(
            feature_dims, nheads, key_dims, value_dims, lig_update_iters, self.device
        )
        self.prolig = ProLig(feature_dims, nheads, pro_lig_update_iters, self.device)

        self.ligand_interaction_loss = nn.BCELoss(
            reduction="sum",
        )
    ...
    def forward(self, res_batch, mol_batch):
        ...
        predicted_lig_interactions, predicted_interactions, predicted_affinities = self.prolig(
            fresidues, fatoms, res_scope, lig_scope
        )
    ...
    def compute_loss(self, predictions, targets, res_batch, mol_batch):
        ...
        for (_, res_count), (_, atom_count), complex_label in zip(
            res_scope, lig_scope, complex_labels
        ):
            l_loss = (
                self.ligand_interaction_loss(
                    predicted_lig_interactions[
                        lig_interaction_count : lig_interaction_count + atom_count**2
                    ],
            ...
            pl_loss = (
                self.pro_lig_interaction_loss(
                    predicted_interactions[
                        pro_lig_interaction_count : pro_lig_interaction_count
                        + atom_count * res_count
                    ],
        ...
        affinity_loss = torch.sum(self.mse_loss(predicted_affinities, pKs) * pK_flags) / torch.sum(
            pK_flags
        )
    ...
    def compute_metrics(predictions, targets):
        ...
        lig_interaction_result = torch.where(predicted_lig_interactions > 0.5, 1.0, 0.0)
        lig_interaction_acc = torch.mean(
            torch.where(lig_interaction_result == mol_intearctions, 1.0, 0.0)
        )

        pro_lig_interaction_result = torch.where(predicted_interactions > 0.5, 1.0, 0.0)
        pro_lig_interaction_acc = torch.mean(
            torch.where(pro_lig_interaction_result == pro_lig_interactions, 1.0, 0.0)
        )

        affinity_mae = torch.sum(torch.abs(predicted_affinities - pKs) * pK_flags) / torch.sum(
            pK_flags
        )
    ...
    def load_parameters(
        self, parameters=os.path.join(os.path.dirname(os.path.abspath(__file__)), "PLANET.param")
    ):
    ...
    def cal_res_features(self, fresidues, batch_size):
        res_scope = [
            (i * int(fresidues.shape[0]), int(fresidues.shape[0])) for i in range(0, batch_size)
        ]
