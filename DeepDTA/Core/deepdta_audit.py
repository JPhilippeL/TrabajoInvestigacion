"""
@file deepdta_audit.py
@brief Split leakage diagnostics for DeepDTA datasets.
"""
...
def _random_splits(dataset_size: int, val_split: float, test_split: float, seed: int) -> dict[str, list[int]]:
...
def get_split_indices(
    ...
    return _random_splits(dataset_size, val_split, test_split, seed), "random"
...
def audit_dataset_splits(
    ...
    if not os.path.exists(ligand_path):
        raise FileNotFoundError(f"Required DeepDTA ligand file not found: {ligand_path}")
    ...
    warnings = [
        "Filtered DeepDTA-compatible subset; raw 378-complex MPro-URV may be reduced by SMILES/protein length constraints.",
        "Test set not used for model selection.",
    ]
    if len(splits["train"]) + len(splits["valid"]) + len(splits["test"]) < 378 and dataset_name == "mpro_urv":
        warnings.append("Filtered DeepDTA-compatible subset contains fewer than 378 raw complexes.")
    ...
    audit = {
        ...
        "raw_count": metadata.get("raw_count", metadata.get("num_raw_complexes", 378 if dataset_name == "mpro_urv" else None)),
        "kept_count": metadata.get("num_ligands", dataset_size),
    ...
    if output_dir:
        ...
        audit_path = os.path.join(output_dir, f"split_audit_deepdta_{timestamp}.json")
