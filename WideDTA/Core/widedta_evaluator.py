"""
@file widedta_evaluator.py
@brief Explicit WideDTA checkpoint evaluation and prediction export.
"""
...
from WideDTA.Core.widedta_audit import audit_dataset_splits, get_split_indices
from WideDTA.Core.widedta_trainer import RMSLoss, evaluate, get_dataset_paths, load_checkpoint, prepare_batch, resolve_device
...
def evaluate_checkpoint(
    ...
    if output_dir is None:
        output_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "results", "widedta_evaluation", "runs", f"run_{timestamp}")
    ...
    ligand_ids = list(json.load(open(ligand_path, encoding="utf-8")).keys())
    protein_ids = list(json.load(open(protein_path, encoding="utf-8")).keys())
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                for pred, true in zip(y_pred, y_true):
                    ...
                    protein_id = protein_ids[0] if protein_ids else ""
                    row = {
    ...
    timestamped_predictions_path = os.path.join(output_dir, f"predictions_widedta_{timestamp}.csv")
    ...
    metrics_payload = {"model": "WideDTA", "metrics_by_split": metrics_by_split, "warnings": audit.get("warnings", [])}
    metrics_path = os.path.join(output_dir, "metrics.json")
    timestamped_metrics_path = os.path.join(output_dir, f"metrics_widedta_{timestamp}.json")
