"""
@file ednn_generate_data.py
@author Mohamed EL BOUKHIARI
@brief Graph generation pipeline for the EDNN model.
@details
This file generates PyTorch Geometric graph files from the URV dataset.
It keeps the same graph-generation logic used for EGNN, but stores graphs
under Graphs_EDNN and adds edge_attr with one distance feature per edge.
"""
...
GRAPH_OUT_DIR = os.path.join(MODULE_ROOT, "Graphs_EDNN")
...
def load_ligand(sdf_path: str):
    ...
    return torch.tensor(coords, dtype=torch.float), torch.tensor(feats, dtype=torch.float)
...
def build_graph(
    ...
    """
    lig_path = os.path.join(LIGAND_SDF_DIR, f"{pdb_id}_ligand.sdf")
    prot_path = os.path.join(PROTEIN_PDB_DIR, f"{pdb_id}_protein.pdb")

    if not os.path.exists(lig_path) or not os.path.exists(prot_path):
        return None

    lig_pos, lig_feat = load_ligand(lig_path)
    if lig_pos is None:
        return None

    prot_pos, prot_feat = load_protein(prot_path)
    ...
    for i in range(pos.shape[0]):
        for j in range(pos.shape[0]):
            if i != j and dist[i, j] < cutoff_edges:
                edges.append([i, j])
                edge_attrs.append([float(dist[i, j])])
...
def generate_all_graphs(
    ...
    """
    @brief Generate all EDNN graphs from the dataset.
    @param pic50_dict Dictionary of pIC50 values.
    ...
    for pdb_id in tqdm(pic50_dict.keys(), desc="Generating EDNN graphs"):
        g = build_graph(
            pdb_id,
            pic50_dict,
...
def generate_data(
    ...
    """
    @brief Main callable function for EDNN graph generation.
    ...
    total_graphs = generate_all_graphs(
        pic50_dict,
        cutoff_edges=cutoff_edges,
        cutoff_prot=cutoff_prot,
    )
...
if __name__ == "__main__":
    results = generate_data(
        ...
        ligand_sdf_dir=LIGAND_SDF_DIR,
        protein_pdb_dir=PROTEIN_PDB_DIR,
        graphs_dir=GRAPH_OUT_DIR,
    )
    print("\nEDNN graph generation completed.")
    print(results)
