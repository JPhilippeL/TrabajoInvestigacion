...
def train_cheapnet(config, train_file, test_file, val_file, log_callback, graph_directory):
    ...
    for split_id in range(len(train_splits)):
        if log_callback:
            log_callback("\n==============================")
            log_callback(f"        SPLIT {split_id:02d}")
            log_callback("==============================")
        ...
        if log_callback:
            log_callback(f"Target mean: {y_mean.item():.4f}")
            log_callback(f"Target std: {y_std.item():.4f}")
        ...
        if log_callback:
            ...
            log_callback(f"Trainable params: {n_params}")
        ...
        for epoch in range(config["EPOCHS"]):
            ...
            if val_rmse < best_val_rmse:
                ...
                if log_callback:
                    log_callback(">>> Nuevo mejor modelo guardado")
        ...
        if log_callback:
            log_callback(f"Best RMSE split {split_id:02d}: {best_val_rmse:.4f}")
            log_callback(f"Test RMSE split {split_id:02d}: {test_rmse:.4f}")
            log_callback(f"Test Pearson split {split_id:02d}: {test_pr:.4f}")
...
def run_hyperparameter_search(
    ...
    if log_callback:
        log_callback(f"Best trial: {best_result.path}")
