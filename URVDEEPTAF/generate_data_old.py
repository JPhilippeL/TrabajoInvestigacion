...
def DB_Generation(
    ...
    """
    Generate database files for model training and evaluation.
    
    Parameters:
    -----------
    dssp_dir : str
        Directory containing protein DSSP files (named as <pdbid>_protein.dssp)
    
    ligand_dir : str
        Directory containing ligand SMI files (named as <pdbid>_ligand.smi)
    
    pocket_file : str
        Path to pocket CSV file with columns: PDB, seq, Positions
    
    output_dir : str, optional
        Output directory where 'data' folder will be created.
        If None, defaults to 'data' in the current directory.
    
    train_ratio : float, optional
        Proportion of data to include in the training set. Default: 0.70
    
    val_ratio : float, optional
        Proportion of data to include in the validation set. Default: 0.15
    
    test_ratio : float, optional
        Proportion of data to include in the test set. Default: 0.15
    ...
    cleanup_processed : bool, optional
        Whether to clean up temporary processed files. Default: True
    
    Returns:
    --------
    dict
        Dictionary containing the counts of files generated in each set
        
    Notes:
    ------
    This function:
    1. Processes protein DSSP files into feature CSVs
    2. Processes pocket information using protein structures
    3. Processes ligand SMILES strings
    4. Splits data into training, validation, and test sets
    5. Creates all necessary output directories and files
    """
    # Start timing
    ...
    # -------------------------
    # Define feature groups and helper functions
    # -------------------------
    c1 = {
    ...
    # -------------------------
    # Parsing the DSSP File (for proteins)
    # -------------------------
    ...
    # -------------------------
    # Protein Processing: Features and Global Sequence Extraction
    # -------------------------
    ...
    # -------------------------
    # Pocket Processing: Features and Sequence Extraction (Updated)
    # -------------------------
    ...
    # -------------------------
    # Ligand Processing: Combine SMI Files
    # -------------------------
    ...
    def split_ligand_csv(ligand_csv, train_ids, val_ids, test_ids):
        """
    ...
    # -------------------------
    def write_details_file(output_path, details):
        """
...
# Main execution when the script is run directly
if __name__ == "__main__":
    ...
    # Required arguments
    parser.add_argument("--dssp_dir", required=True, help="Directory containing protein DSSP files")
    parser.add_argument("--ligand_dir", required=True, help="Directory containing ligand SMI files")
    parser.add_argument("--pocket_file", required=True, help="Path to pocket CSV file")
    
    # Optional arguments
    parser.add_argument("--output_dir", default=None, help="Output directory (defaults to './data')")
    ...
    parser.add_argument("--keep_processed", action="store_true", help="Keep temporary processing files")
    ...
    DB_Generation(
        ...
        cleanup_processed=not args.keep_processed
    )