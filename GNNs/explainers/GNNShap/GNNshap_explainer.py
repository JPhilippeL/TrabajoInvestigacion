...
from .utils import *
from .explanation import GNNShapExplanation
...
class GNNShapExplainer:
    ...
    def __compute_preds_no_batching(self, node_features: Tensor,
                                    edge_index: Tensor, mask_matrix: torch.BoolTensor,
                                    node_idx: int) -> Tensor:
        ...
        for i in tqdm(range(mask_matrix.shape[0]), desc="Coalition scores",
            ...
            # CAMBIO PARA REGRESIÓN: Extraemos el valor continuo
            preds[i] = y_hat.squeeze().item()
    ...
    def __compute_preds_batched(self, node_features: Tensor, edge_index: Tensor,
        ...
        for i in tqdm(range(num_batches), desc="Batched coalition scores",
            ...
            # CAMBIO PARA REGRESIÓN: Usar squeeze() en lugar de indexar por target_class
            preds[current_ind: current_ind + tmp_batch_size] = y_hat.squeeze()
    ...
    def compute_model_predictions(self, node_features: Tensor, edge_index: Tensor,
                              mask_matrix: torch.BoolTensor, node_idx: int,
                              batch_size: int) -> Tuple[Tensor, int]:
        ...
        # Extraemos el valor continuo directamente con squeeze().item()
    ...
    def explain(self, node_idx: int, nsamples: int,
        ...
        logger.info(f"Computational graph finding time(s):\t{compgraph_time -  start_time:.4f}")
        ...
        logger.info(f"Sampling time(s):\t\t{sampling_time - compgraph_time:.4f}")
        ...
        pred_time = time.time()
        logger.info(f"Model predictions time(s):{pred_time - sampling_time:.4f}")
        ...
        if nplayers > 5000 and solver_name == "WLSSolver" and device != 'cpu':
            ...
            logger.warning(f"Switching to WLRSolver. Reason: large number of players: {nplayers}")
        ...
        logger.info(f"Solve time(s):\t{solve_time -  pred_time:.4f}")
    ...
    def explain_regression(self, node_idx: int, nsamples: int,
                        batch_size: int = 512, sampler_name: str = 'GNNShapSampler',
                        l1_reg: bool= False, solver_name: str = "WLSSolver", **kwargs):
        device = self.device
        start_time = time.time()

        # 1. Extracción del subgrafo (igual que antes)
        (subset, sub_edge_index, sub_mapping,
        sub_edge_mask) = pruned_comp_graph(node_idx, self.num_hops,
        ...
        # 2. Muestreo de coaliciones (igual que antes)
        self.sampler = get_sampler(
            sampler_name=sampler_name, nplayers=nplayers, nsamples=nsamples,
            edge_index=sub_edge_index, nhops=self.num_hops,
            target_node=sub_mapping[0].item(), **kwargs)
        ...
        mask_matrix = mask_matrix.to(device)
        kernel_weights = kernel_weights.to(device)
        nsamples = self.sampler.nsamples 
        sampling_time = time.time()

        # Manejo de self-loops (igual)
        use_add_self_loops = has_add_self_loops(self.model)
        add_self_loops_swithced = False
        if use_add_self_loops:
            switch_add_self_loops(self.model)
        ...
        # 3. PREDICCIONES DEL MODELO (Modificado: sin target_class)
        # Aquí asegúrate de que compute_model_predictions devuelva el valor continuo
        preds, comp_samp = self.compute_model_predictions(node_features, self_loop_sub_edge_index,
                                            torch_self_loop_mask_matrix_bool,
        ...
        # El solver no necesita saber si es clasificación o regresión, 
        # solo resuelve un sistema de ecuaciones lineales usando los outputs numéricos (preds).
        solver = get_solver(solver_name=solver_name, mask_matrix=mask_matrix,
                            kernel_weights=kernel_weights, yhat=preds, fnull=self.fnull,
                            ffull=self.fx, device=device, fraction_evaluated=fraction_evaluated,
                            l1_reg=l1_reg)
        ...
        # 4. GNNShapExplanation (Modificado: pasamos None o un placeholder en target_class)
        explanation = GNNShapExplanation(node_idx, nplayers, float(self.fnull), shap_vals, nsamples,
                                        self.fx, None, sub_edge_index, subset,
                                        self.data.y[subset],
                                        time_total_comp=total_time,
                                        time_comp_graph=compgraph_time -  start_time,
                                        time_sampling=sampling_time - compgraph_time,
