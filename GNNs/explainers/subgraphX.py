...
from dig.xgraph.method.shapley import gnn_score, mc_shapley, l_shapley, mc_l_shapley, NC_mc_l_shapley, sparsity
...
class SubgraphX(object):
    ...
    # CAMBIADO:
    # Por qué falla: En la primera línea del método explain, el código original hace esto:
    # probs = self.model(x, edge_index).squeeze().softmax(dim=-1).
    # La función softmax convierte un vector de números reales en probabilidades que suman 1.
    # Si tu red de regresión devuelve un único valor (por ejemplo, 8.5), 
    # hacerle un softmax en esa dimensión lo convertirá instantáneamente en 1.0. 
    # Destruye la salida continua de tu modelo y hace que la predicción de todos los subgrafos sea idéntica (1.0).
    # Qué cambiar: Hay que quitar el softmax y capturar la predicción cruda.
