...
class PredictDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Predict GRAPHDTA")
        self.resize(700, 560)

        self.settings = QSettings("Investigacion", "Predict_GRAPHDTA")

        self.test_split_input = QLineEdit()
        self.test_split_input.setPlaceholderText("Test split file (.txt)")
        self.test_split_input.setText(self.settings.value("train_graphdta/last_test_file", ""))
    ...
    def accept(self):
        ...
        self.settings.setValue(
            "train_graphdta/last_test_file", self.test_split_input.text().strip()
        )

        self.settings.setValue("predict/last_model_dir", self.model_dir_input.text().strip())
        self.settings.setValue("train_graphdta/last_graph_dir", self.graph_dir_input.text().strip())
