...
BLOSUM62 = {
    ...
    "ASN": np.array(
        [[-2, 0, 6, 1, -3, 0, 0, 0, 1, -3, -3, 0, -2, -3, -2, 1, 0, -4, -2, -3]], dtype=np.float32
    ),
    "ASP": np.array(
    ...
    "GLN": np.array(
        [[-1, 1, 0, 0, -3, 5, 2, -2, 0, -3, -2, 1, 0, -3, -1, 0, -1, -2, -1, -2]], dtype=np.float32
    ),
    "GLU": np.array(
        [[-1, 0, 0, 2, -4, 2, 5, -2, 0, -3, -3, 1, -2, -3, -1, 0, -1, -3, -2, -2]], dtype=np.float32
    ),
    "GLY": np.array(
    ...
    "SER": np.array(
        [[1, -1, 1, 0, -1, 0, 0, 0, -1, -2, -2, 0, -1, -2, -1, 4, 1, -3, -2, -2]], dtype=np.float32
    ),
    "THR": np.array(
...
def atom_features(atom):
    return torch.Tensor(
        onek_encoding_unk(atom.GetSymbol(), LIGADND_ELEMENTS)
...
def mass_center_from_pdb(pdb_lines):
    ...
    atoms_mass = np.reshape(
        np.array(
            [periodic_table.GetAtomicWeight(line[76:78].strip().capitalize()) for line in pdb_lines]
        ),
        [-1, 1],
    )
    coordinates = np.array(
        [[float(line[30:38]), float(line[38:46]), float(line[46:54])] for line in pdb_lines],
        dtype=np.float32,
    )
...
class ProteinPocket:
    def __init__(
        self, protein_pdb, centeriod_x=None, centeriod_y=None, centeriod_z=None, ligand_sdf=None
    ):
        ...
        self.res_features = torch.from_numpy(
            np.concatenate([residue.get_feature() for residue in self.pocket_residues], axis=0)
        )
        self.alpha_coordinates = torch.from_numpy(
            np.concatenate(
                [residue.get_alpha_position() for residue in self.pocket_residues], axis=0
            )
        )
...
class ComplexPocket:
    def __init__(self, protein_pdb, ligand_sdf, pK=0, decoy_sdf=None):
        ...
        if decoy_sdf is not None and os.path.exists(decoy_sdf):
            self.decoys = [
                Mol(mol) for mol in Chem.SDMolSupplier(decoy_sdf, removeHs=False) if mol is not None
            ]
        else:
            self.decoys = []
        ...
        self.pocket_residues = []
        pocket_residues_id = []
        pocket_atoms_content = [
            line for line in pdb_content if near_pocket(line, self.ligand.compute_centeroid())
        ]
        ...
        self.res_features = np.concatenate(
            [residue.get_feature() for residue in self.pocket_residues], axis=0
        )
        self.res_count = len(self.pocket_residues)
        self.distance_matrix = self.get_distance_matrix()

    def get_interaction_label(self):
        pro_lig_interaction = np.zeros(
            shape=[self.ligand.mol.GetNumAtoms(), len(self.pocket_residues)], dtype=np.float32
        )
        ...
        for atom in self.ligand.mol.GetAtoms():
            ...
            atom_coordinate = np.array(
                [
                    ligand_conformer.GetAtomPosition(atom_index)[0],
                    ligand_conformer.GetAtomPosition(atom_index)[1],
    ...
    def get_distance_matrix(self):
        mass_coordinates = np.array(
            [residue.get_mass_center() for residue in self.pocket_residues], dtype=np.float32
        )
        alpha_coordinates = np.array(
            [residue.get_alpha_position() for residue in self.pocket_residues], dtype=np.float32
        )
        mass_matrix = np.sqrt(
            np.sum(
                np.square(
...
class Residue:
    def __init__(self, residue_content):
        ...
        self.mass_center, self.coordinates, self.alpha_position = mass_center_from_pdb(
            residue_content
        )
    ...
    def process_alternate(self, contents):
        ...
        if len(alternate_flag) > 0:
            contents = [
                line for line in contents if line[16] == alternate_flag[0] or line[16] == " "
            ]
        return contents
...
class Mol:
    ...
    def get_atom_coordinates(self):
        ...
        atom_coordinates = np.array(
            [
                [
                    ligand_conformer.GetAtomPosition(atom.GetIdx())[0],
                    ligand_conformer.GetAtomPosition(atom.GetIdx())[1],
                    ligand_conformer.GetAtomPosition(atom.GetIdx())[2],
                ]
    ...
    def get_interaction_label(self):
        ...
        atom_distances = np.sqrt(
            np.sum(
                np.square(atom_coordinates.reshape(-1, 1, 3) - atom_coordinates.reshape(1, -1, 3)),
                axis=-1,
            )
        )
        interaction = np.where(atom_distances < 4.0, 1.0, 0.0)
        return interaction
...
def tensorize_all(protein_pockets, decoy_flag=True):
    ...
    for i, label in enumerate(complex_labels):
        if label == 1:
            ...
            pro_lig_interactions.append(
                np.zeros([mol_batch[i].atom_count, protein_pockets[i].res_count]).reshape([-1])
            )
...
def tensorize_protein_pocket(protein_pockets):
    ...
    for pocket in protein_pockets:
        ...
        res_map[
            total_residues : total_residues + residue_count,
            total_residues : total_residues + residue_count,
        ] = 1.0
    ...
    alpha_coordinates = torch.from_numpy(
        np.reshape(np.array(alpha_coordinates, dtype=np.float32), [-1, 3])
    )
...
def role_of_5(mol):
    ...
    qed_score = qed(mol)
    meet = [
        mol_weight >= 300 and mol_weight <= 500,
        mol_logp < 5,
        mol_ha <= 10,
        mol_hd <= 5,
        mol_rb <= 10,
