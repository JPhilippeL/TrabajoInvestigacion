...
class MenuEGNN(QMenu):
    ...
    def generate_data_egnn(self):
        ...
        if dialog.exec():
            ...
            logger.info("Starting EGNN graph generation in background...")
    ...
    def train_model_egnn(self):
        ...
        if dialog.exec():
            ...
            logger.info("Starting EGNN training in background...")
            self.main_window.setEnabled(False)
    ...
    def train_all_models_egnn(self):
        ...
        if dialog.exec():
            ...
            if (
                ...
                logger.warning("Missing required paths for EGNN batch training / hyperparameter search.")
                return

            logger.info("Starting EGNN batch training / hyperparameter search...")
            self.main_window.setEnabled(False)
    ...
    def test_model_egnn(self):
        ...
        if dialog.exec():
            ...
            logger.info("Starting EGNN evaluation in background...")
            self.main_window.setEnabled(False)
    ...
    def on_generation_success(self, results):
        self.main_window.setEnabled(True)
        logger.info(f"EGNN data generation completed: {results}")

    def on_train_success(self, run_dir):
        self.main_window.setEnabled(True)
        logger.info(f"EGNN training completed. Results saved in: {run_dir}")

    def on_batch_train_success(self, results):
        self.main_window.setEnabled(True)
        logger.info(f"EGNN batch training / hyperparameter search completed: {results}")
    ...
    def on_test_success(self, metrics):
        ...
        logger.info(f"EGNN evaluation completed. Metrics: {metrics_log}")

    def on_batch_test_model_success(self, model_name, metrics):
        metrics_log = " | ".join(
            [f"{k}: {v:.4f}" if isinstance(v, (float, int)) else f"{k}: {v}" for k, v in metrics.items()]
        )
        logger.info(f"[EGNN Batch Test] {model_name} completed. Metrics: {metrics_log}")

    def on_batch_test_all_finished(self, csv_path):
        self.main_window.setEnabled(True)
        if csv_path:
            logger.info(f"EGNN batch evaluation finished. Summary saved in: {csv_path}")
        else:
