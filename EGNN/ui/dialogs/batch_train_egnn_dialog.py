...
class BatchTrainDialog(QDialog):
    ...
    def browse_graphs(self):
        path = QFileDialog.getExistingDirectory(self, "Seleccionar carpeta Graphs_EGNN")
        if path:
            self.graphs_dir_input.setText(path)

    def browse_train_split(self):
        path, _ = QFileDialog.getOpenFileName(self, "Seleccionar train split", "", "Text Files (*.txt);;All Files (*)")
        if path:
            self.train_split_input.setText(path)

    def browse_val_split(self):
        path, _ = QFileDialog.getOpenFileName(self, "Seleccionar validation split", "", "Text Files (*.txt);;All Files (*)")
        if path:
            self.val_split_input.setText(path)

    def browse_test_split(self):
        path, _ = QFileDialog.getOpenFileName(self, "Seleccionar test split", "", "Text Files (*.txt);;All Files (*)")
        if path:
            self.test_split_input.setText(path)

    def browse_models_root(self):
        path = QFileDialog.getExistingDirectory(self, "Seleccionar carpeta raíz de modelos")
        if path:
            self.models_root_input.setText(path)

    def browse_results_root(self):
        path = QFileDialog.getExistingDirectory(self, "Seleccionar carpeta raíz de resultados")
        if path:
