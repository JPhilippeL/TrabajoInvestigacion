...
class HyperparameterSearchDEAttentionDTADialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.lr_values_input = QLineEdit(self.settings.value("hpo/lr_values", "0.00005,0.0001"))
        self.batch_size_values_input = QLineEdit(self.settings.value("hpo/batch_size_values", "8,16"))
        self.weight_decay_values_input = QLineEdit(self.settings.value("hpo/weight_decay_values", "0.0,0.01"))
    ...
    def accept(self) -> None:
        for key, value in self.get_inputs().items(): self.settings.setValue(f"hpo/{key}", value)
        super().accept()
