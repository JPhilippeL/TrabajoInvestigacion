...
def _build_grid(params: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for lr, batch_size, weight_decay in itertools.product(learning_rates, batch_sizes, weight_decays):
        ...
        grid.append({"lr": float(lr), "batch_size": int(batch_size), "weight_decay": float(weight_decay)})
    return grid
...
def _run_one_split(
    *,
    ...
    train_dataset = base.URVDEAttentionDataset(paths["train_seq"], paths["train_aff"], base.DEFAULT_MAX_SEQ_LEN, base.DEFAULT_MAX_SMI_LEN)
    valid_dataset = base.URVDEAttentionDataset(paths["valid_seq"], paths["valid_aff"], base.DEFAULT_MAX_SEQ_LEN, base.DEFAULT_MAX_SMI_LEN)
    train_loader = DataLoader(train_dataset, batch_size=int(hparams["batch_size"]), shuffle=True, num_workers=num_workers)
    valid_loader = DataLoader(valid_dataset, batch_size=int(hparams["batch_size"]), shuffle=False, num_workers=num_workers)
    ...
    print(
        f"{trial_id} split {split_id:02d} | train={len(train_dataset)} valid={len(valid_dataset)} "
        f"lr={hparams['lr']} batch={hparams['batch_size']} wd={hparams['weight_decay']} device={device}"
    )

    for epoch in range(1, epochs + 1):
        train_loss = _train_one_epoch(model, train_loader, optimizer, criterion, device, grad_clip_norm)
        ...
        print(
            ...
            f"val_RMSE={val_metrics['RMSE']:.6f} val_Pearson={val_metrics['Pearson']:.6f}"
            + (" [best]" if improved else "")
        )
        if epoch >= min_epochs_before_stopping and rounds_without_improvement >= early_stopping_rounds:
            print(f"{trial_id} split {split_id:02d} early stopping at epoch {epoch}; best_epoch={best_epoch}")
            break

    if best_state is None or best_metrics is None:
        raise DEAttentionDTAHPOError(f"{trial_id} split {split_id} did not produce a valid checkpoint.")
    ...
    torch.save(
        {
            ...
            "metrics": {"validation": best_metrics},
            "test_rows_used": 0,
        },
        model_path,
    )
...
def run_hyperparameter_search(params: Mapping[str, Any]) -> dict[str, Any]:
    ...
    prepared_dir = Path(as_absolute_string(str(params.get("prepared_dir", "")), must_exist=True))
            ...
            row.update(
                {
                    ...
                    "elapsed_seconds": float(frame["elapsed_seconds"].sum()),
                }
            )
    ...
    if best_row is None:
        raise DEAttentionDTAHPOError("All DEAttentionDTA HPO trials failed. Inspect deattentiondta_hpo_trials.csv.")
